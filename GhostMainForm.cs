...
namespace WindowsFormsApplication4
{
    public partial class GhostMainForm : Form
    {
        ...
        {
            List<String> PopularAutors = SQLClass.Select(
                    "SELECT UserName FROM " + Tables.AUTHORS +
                    " WHERE locked = 0 LIMIT 0, 3");

            int articleY = 200;
            for (int artIndex = 0; artIndex < PopularAutors.Count; artIndex++)
            {
                Label label1 = new Label();
                label1.Location = new Point(28, articleY);
                label1.Size = new Size(Right_panel.Width, 20);
                label1.Text = PopularAutors[artIndex].ToString();
